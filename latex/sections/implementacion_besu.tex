...
contrato, sus funciones principales y las consideraciones técnicas encontradas durante el despliegue.

\subsection{Arquitectura del contrato}

El contrato \texttt{ContratoONG.sol} implementa un diseño modular, donde cada parte del 
...
\textbf{OngBase:} Define las estructuras de datos y mapeos básicos para donantes, proyectos, donaciones, compras y proveedores:
...
\begin{itemize}
    ...
    actualiza totales y asigna tokens de gobernanza (1 token por cada 0.001 ETH). Si el donante no está registrado, lo crea automáticamente como individuo. Solo permite donar a proyectos en estado Activo.

    \item \texttt{votarAprobacion(string proyectoId, uint256 cantidadVotos)}: Permite a los donantes registrados votar para aprobar un proyecto en estado Propuesto. Los tokens de gobernanza se queman al votar. Si se alcanza el umbral de votos mínimos (VOTOS\_MINIMOS = 2), el proyecto pasa a estado Activo.
    ...
    \item \texttt{realizarCompra(string compraId, string proyectoId, address proveedor, string tipoMaterial, uint128 cantidad)}: Permite al responsable del proyecto realizar compras. Verifica fondos suficientes y reserva el monto del balance del proyecto.
...
El sistema incluye un contrato separado \texttt{TokenGobernanza.sol} que implementa un token ERC20 para la gobernanza de la ONG. Este token permite a los donantes participar en las votaciones de proyectos.
...
\begin{itemize}
    ...
    \item \textbf{Control de acceso}: Solo el contrato de donaciones (ContratoONG) o el owner pueden mintear y quemar tokens
\end{itemize}
